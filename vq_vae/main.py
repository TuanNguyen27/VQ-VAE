...
def main(args):
    ...
    training_parser.add_argument('--dataset', default='mnist', choices=['mnist', 'Fashionmnist'],
...
def train(epoch, model, train_loader, optimizer, cuda, log_interval, save_path, args):
    ...
    return torch.stack(full_mse_losses).reshape(-1)
...
def test_net(epoch, model, test_loader, cuda, save_path, args):
    ...
    with torch.no_grad():
        for i, (data, _) in enumerate(test_loader):
            ...
            latest_losses = model.latest_losses()
            full_mse_losses.append(latest_losses['full_mse'])
    print(torch.stack(full_mse_losses).reshape(-1).size())
    return torch.stack(full_mse_losses).reshape(-1)
            # for key in latest_losses:
